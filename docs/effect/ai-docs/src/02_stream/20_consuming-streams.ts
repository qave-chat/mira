...
export const paidOrders = normalizedOrders.pipe(Stream.filter((order) => order.status === "paid"));
...
export const firstTwoOrders = enrichedPaidOrders.pipe(Stream.take(2), Stream.runCollect);

export const afterWarmupOrder = enrichedPaidOrders.pipe(Stream.drop(1), Stream.runCollect);
